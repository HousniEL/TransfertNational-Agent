...
export class MainDebitComponent implements OnInit {
  ...
  constructor( public transferT : TransferService, private formBuilder : FormBuilder ,
    private multitransferSrv : MultitransfersService,private agentServ:AgentService) {
  ...
  submit(){
    ...
    setTimeout(() => {
      ...
      this.transferT.transfer(this.transfer.controls.motif.value, Number(this.transfer.controls.frais.value));

      this.agentServ.checkAccount_solde(this.transferT.multiTransfer.id_client,this.transferT.multiTransfer.total_amount).subscribe((data:any)=>{
        this.multitransferSrv.setTransfer(this.transferT.multiTransfer).subscribe((data:any)=>{
          transfer.innerHTML = "Transférer";
          transfer.removeAttribute("disabled");
          console.log(data);
